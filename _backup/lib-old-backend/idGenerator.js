 ...
 * This is the RECOMMENDED function for all investment creation
 * @returns {Promise<string>} - Next investment ID globally (e.g., "INV-10001")
 */
export async function generateSequentialInvestmentId() {
  try {
    ...
    if (error) {
      ...
      throw new Error(`Failed to query investments: ${error.message}`)
    }
    ...
    console.error('Error generating sequential investment ID:', error)
    throw new Error(`Failed to generate investment ID: ${error.message}`)
  }
}
  ...
  switch (transactionType) {
    ...
    case 'contribution':
      if (options.date) {
        ...
        return `${prefix}-CONTR-${year}-${month}`
      }
      return `${prefix}-CONTR-${Date.now()}`
    ...
    case 'distribution':
      if (options.date) {
        ...
        const year = date.getUTCFullYear()
        const month = String(date.getUTCMonth() + 1).padStart(2, '0')
        return `${prefix}-DIST-${year}-${month}`
      }
      return `${prefix}-DIST-${Date.now()}`
    
    case 'redemption':
      if (options.withdrawalId) {
        return `${prefix}-REDEEM-${options.withdrawalId}`
      }
      if (options.date) {
        const date = new Date(options.date)
        const year = date.getUTCFullYear()
        ...
        return `${prefix}-REDEEM-${year}-${month}-${day}`
      }
      return `${prefix}-REDEEM-${Date.now()}`
...
/**
 * Validate investment ID format (strict sequential format)
 * @param {string} id - Investment ID to validate
 * @returns {boolean} - True if valid sequential format (INV-{digits})
 */
export function validateInvestmentId(id) {
  if (!id || typeof id !== 'string') return false
  const pattern = /^INV-\d+$/
...
export default {
  ...
  generateSequentialInvestmentId,
