 ...
 * Uses a lightweight select to detect PostgREST schema errors without requiring extra SQL functions.
 * Returns false if Supabase reports the column is missing.
 */
async function tableColumnExists(supabase, tableName, columnName) {
  try {
    const { error } = await supabase.from(tableName).select(columnName).limit(1)
    if (error) {
      const msg = `${error.message || ''} ${error.details || ''}`.toLowerCase()
      // PostgREST typically returns: "Could not find the 'address' column of 'users' in the schema cache"
      if (msg.includes(`'${columnName}'`) && msg.includes('column') && msg.includes(tableName)) {
        return false
      }
    }
    return true
  } catch (e) {
    // If anything unexpected happens, assume column may not exist to avoid hard failure on write
    return false
  }
}
...
export async function addUser(userData) {
  try {
    ...
    // Only include address if the column exists in the target database
    if (Object.prototype.hasOwnProperty.call(userData, 'address')) {
      const hasAddressColumn = await tableColumnExists(supabase, 'users', 'address')
      if (hasAddressColumn) {
        newUser.address = userData.address || null
      } else {
        console.warn("'users.address' column not found; proceeding without address for new user", normalizedEmail)
      }
    }
...
export async function updateUser(userId, updateData) {
  try {
    ...
    // Guard: If attempting to update address but column is missing, return a clear error
    if (Object.prototype.hasOwnProperty.call(dbData, 'address')) {
      const hasAddressColumn = await tableColumnExists(supabase, 'users', 'address')
      if (!hasAddressColumn) {
        return { success: false, error: "Address field is not available in the database. Ask an admin to add column 'users.address' (JSONB)." }
      }
    }
  ...
  try {
    ...
    if (data && data.length > 0) {
      ...
      allTransactions.forEach(tx => {
        ...
        // Include metadata JSONB if it exists
        if (tx.metadata) {
          transaction.metadata = tx.metadata
          // Extract distributionTxId from metadata for contributions (critical reference field)
          if (tx.type === 'contribution' && tx.metadata.distributionTxId) {
            transaction.distributionTxId = tx.metadata.distributionTxId
          }
        }
        ...
        // Include created_at timestamp if it exists
        if (tx.created_at) {
          transaction.createdAt = tx.created_at
        }
...
export async function addInvestment(userId, investmentData) {
  try {
    ...
    // If this is the user's first investment, set their account type
    if (investmentData.accountType) {
      // Check if user already has an account type set
      ...
      console.log(`Checking user ${userId} account type:`, {
        currentAccountType: existingUser?.account_type,
        newAccountType: investmentData.accountType
      })

      // Only set account type if user doesn't already have one
      if (!existingUser?.account_type) {
        console.log(`Setting user ${userId} account type to ${investmentData.accountType}`)
        ...
        if (updateError) {
          console.error(`Failed to set user account type:`, updateError)
        } else {
          console.log(`✅ Successfully set user ${userId} account type to ${investmentData.accountType}`)
        }
      } else {
        console.log(`User ${userId} already has account type: ${existingUser.account_type}`)
      }
    }
...
export async function updateInvestment(investmentId, updateData) {
  try {
    ...
    // If account type is being set and user doesn't already have one, update user's account type
    if (updateData.accountType && currentInvestment && currentInvestment.user_id) {
      // Check if user already has an account type set
      const { data: existingUser } = await supabase
        .from('users')
        .select('account_type')
        .eq('id', currentInvestment.user_id)
        .maybeSingle()

      console.log(`Update investment - checking user ${currentInvestment.user_id} account type:`, {
        currentAccountType: existingUser?.account_type,
        newAccountType: updateData.accountType
      })

      // Only set account type if user doesn't already have one
      if (!existingUser?.account_type) {
        console.log(`Setting user ${currentInvestment.user_id} account type to ${updateData.accountType}`)
        const { error: updateError } = await supabase
          .from('users')
          .update({
        ...
        if (updateError) {
          ...
          console.log(`✅ Successfully set user ${currentInvestment.user_id} account type to ${updateData.accountType}`)
        }
      } else {
        console.log(`User ${currentInvestment.user_id} already has account type: ${existingUser.account_type}`)
      }
    }
