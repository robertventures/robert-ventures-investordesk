...
// Supabase migration: Master password now stored in Supabase or local file system
// @netlify/blobs is no longer used
const useBlobs = false // Disabled after Supabase migration
...
async function storeMasterPasswordData(data) {
  if (useBlobs) {
    ...
    if (typeof store.setJSON === 'function') {
      await store.setJSON('master-password.json', data)
    } else {
      await store.set('master-password.json', JSON.stringify(data), { contentType: 'application/json' })
    }
  } else {
    // Use file-based storage for local development to persist across hot reloads
    try {
...
async function getMasterPasswordData() {
  if (useBlobs) {
    const store = getStore({ name: 'users' })
    let data = await store.get('master-password.json', { type: 'json' })
    if (!data && typeof store.getJSON === 'function') {
      try {
        data = await store.getJSON('master-password.json')
      } catch (e) {
        console.log('No master password data found')
      }
    }
    return data
  } else {
    ...
    // Then try file-based storage for local development
    try {
