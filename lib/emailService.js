 ...
 */
export async function sendDocumentNotification({ email, firstName, lastName }) {
  if (!isEmailConfigured()) {
    console.warn('⚠️  Email service not configured. Skipping document notification email.')
    return { success: false, error: 'Email service not configured' }
  }

  const subject = `Your Documents Are Ready - Robert Ventures`
  const html = generateDocumentNotificationHTML({ firstName, lastName })
  const text = generateDocumentNotificationText({ firstName, lastName })
  ...
  try {
    ...
    console.log(`Document notification sent to ${email}:`, result)
    return { success: true, messageId: result.id }
  } catch (error) {
    console.error(`Failed to send document notification to ${email}:`, error)
    return { success: false, error: error.message }
  }
}
  ...
  <div class="content">
    ...
    <div class="steps">
      ...
      <div class="step">
        ...
        <strong>Verify Your SSN</strong> - Required for identity verification
      </div>
      <div class="step">
...
function generateDocumentNotificationHTML({ firstName, lastName }) {
...
<head>
  <meta charset="UTF-8">
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
  <title>Documents Available</title>
  <style>
    body {
      font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, 'Helvetica Neue', Arial, sans-serif;
      line-height: 1.6;
...
<body>
  ...
  <div class="content">
    ...
    <p>Your documents are now available for download in the Robert Ventures Investor Portal.</p>
    ...
    <ul>
      <li>Please download and save these documents for your records</li>
      <li>Keep these documents in a secure location</li>
      <li>Contact us if you notice any discrepancies</li>
    </ul>
    
    <p>If you have any questions about your documents, please don't hesitate to reach out to our team.</p>
...
function generateDocumentNotificationText({ firstName, lastName }) {
  const documentsUrl = `${appUrl}/dashboard?view=documents`
...
Your documents are now available for download in the Robert Ventures Investor Portal.

Document Type: PDF Document
Status: Ready for Download

To access your documents, visit:
${documentsUrl}

Important Notes:
- Please download and save these documents for your records
- Keep these documents in a secure location
- Contact us if you notice any discrepancies

If you have any questions about your documents, please don't hesitate to reach out to our team.
