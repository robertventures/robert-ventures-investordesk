...
/**
 * Send tax document notification email
 */
export async function sendTaxDocumentNotification({ email, firstName, lastName }) {
  const subject = `Your Tax Documents Are Ready - Robert Ventures`
  const html = generateTaxDocumentHTML({ firstName, lastName })
  const text = generateTaxDocumentText({ firstName, lastName })
  ...
  try {
    ...
    console.log(`Tax document notification sent to ${email}:`, result)
    return { success: true, messageId: result.id }
  } catch (error) {
    console.error(`Failed to send tax document notification to ${email}:`, error)
    return { success: false, error: error.message }
  }
}
...
function generateTaxDocumentHTML({ firstName, lastName }) {
  const documentsUrl = `${appUrl}/dashboard?view=documents`
...
<head>
  <meta charset="UTF-8">
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
  <title>Tax Documents Available</title>
  <style>
    body {
      font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, 'Helvetica Neue', Arial, sans-serif;
      line-height: 1.6;
...
<body>
  ...
  <div class="content">
    ...
    <p>Your tax documents are now available for download in the Robert Ventures Investor Portal.</p>
    
    <div class="info-box">
      <p><strong>Document Type:</strong> Tax Form</p>
      <p style="margin-bottom: 0;"><strong>Status:</strong> Ready for Download</p>
    </div>
    
    <p>To access your tax documents:</p>
    
    <div style="text-align: center;">
      <a href="${documentsUrl}" class="button">View Documents</a>
    </div>
    
    <p><strong>Important Notes:</strong></p>
    <ul>
      <li>Please download and save these documents for your tax records</li>
      <li>Consult with your tax advisor for filing guidance</li>
      <li>Keep these documents for at least 7 years</li>
      <li>Contact us if you notice any discrepancies</li>
    </ul>
    
    <p>If you have any questions about your tax documents, please don't hesitate to reach out to our team.</p>
  ...
  <div class="footer">
    <p>© ${new Date().getFullYear()} Robert Ventures. All rights reserved.</p>
    <p>If the button above doesn't work, copy and paste this link into your browser:<br>
    <a href="${documentsUrl}">${documentsUrl}</a></p>
  </div>
</body>
</html>
  `
...
function generateTaxDocumentText({ firstName, lastName }) {
  const documentsUrl = `${appUrl}/dashboard?view=documents`
...
Your tax documents are now available for download in the Robert Ventures Investor Portal.

Document Type: Tax Form
Status: Ready for Download

To access your tax documents, visit:
${documentsUrl}

Important Notes:
- Please download and save these documents for your tax records
- Consult with your tax advisor for filing guidance
- Keep these documents for at least 7 years
- Contact us if you notice any discrepancies

If you have any questions about your tax documents, please don't hesitate to reach out to our team.
