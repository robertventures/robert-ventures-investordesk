...
import { initializeTaxInfo } from './taxCompliance.js'
...
function createTestUser(userId, accountType, paymentFreq, lockup, index) {
  ...
  // Create account creation timestamp (base timestamp)
  ...
  const user = {
    ...
    investments: [],
    taxInfo: initializeTaxInfo({ ssn: accountData.ssn }),
    taxEvents: [],
  ...
  // For test accounts, mark SSN as verified to avoid backup withholding in testing
  user.taxInfo.ssnVerified = true
  user.taxInfo.ssnVerifiedDate = accountCreatedTimestamp
  user.taxInfo.w9OnFile = true
  user.taxInfo.w9ReceivedDate = accountCreatedTimestamp
  ...
  if (accountType === 'joint') {
    ...
    // Entities use EIN instead of SSN
    user.taxInfo.tinType = 'EIN'
...
function createInvestment(investmentId, userId, accountType, paymentFreq, lockup, amount, accountCreatedTimestamp) {
  ...
  // Investment confirmed 10 seconds after account (5 seconds after created)
  ...
  const investment = {
    ...
    bankApprovedAt: investmentConfirmedTimestamp,
    adminApproved: true,
    adminApprovedBy: 'system',
    adminApprovedAt: investmentConfirmedTimestamp,
