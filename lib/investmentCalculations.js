...
export function calculateInvestmentValue(investment, asOfDate = null) {
  ...
  const totalMonthsElapsed = calculateMonthsElapsed(confirmedDate, currentDate)
  ...
  if (investment.paymentFrequency === 'compounding') {
    ...
    // Monthly interest payments (interest paid out, principal remains)
    totalEarnings = investment.amount * monthlyRate * totalMonthsElapsed
    // For display purposes, show what would have been earned
    currentValue = investment.amount + totalEarnings
  }
 ...
 * @returns {Object}
 */
export function getInvestmentStatus(investment) {
