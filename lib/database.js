...
export async function getUsers(retries = 3, delayMs = 1000, expectedTimestamp = null) {
  try {
    if (useBlobs) {
...
export async function saveUsers(usersData) {
  try {
    if (useBlobs) {
      ...
      // Verify write consistency by reading back with expected timestamp
      // This ensures subsequent reads will get the updated data
      // Using 5 retries with 1-second delays for better consistency
      const verification = await getUsers(5, 1000, saveTimestamp)
      ...
      if (!verification || !verification.lastModified) {
        ...
        // Add extra delay as fallback for safety
