...
export async function getUser(userId, skipCache = false) {
  try {
    ...
    // Cache the result
    cache.set(cacheKey, data, CACHE_TTL.USER)
...
export async function getUserByEmail(email, skipCache = false) {
  try {
    ...
    // Check cache first
    const cacheKey = CACHE_KEYS.USER_BY_EMAIL(normalizedEmail)
    if (!skipCache) {
      const cached = cache.get(cacheKey)
      if (cached) return cached
    }
    ...
    if (error) {
      if (error.code === 'PGRST116') {
        // No rows found - cache negative result too
        cache.set(cacheKey, null, CACHE_TTL.USER)
        return null
      }
...
export async function addUser(userData) {
  try {
    ...
    cache.delete(CACHE_KEYS.USER_BY_EMAIL(normalizedEmail))
...
export async function updateUser(userId, updateData) {
  try {
    ...
    if (data && data.email) {
      cache.delete(CACHE_KEYS.USER_BY_EMAIL(data.email))
    }
 ...
 * PERFORMANCE: Uses aggressive caching and optimized batch queries
 * @param {boolean} skipCache - Skip cache and force fresh data
...
export async function getUsers(skipCache = false) {
  try {
    ...
    // Cache the result
    cache.set(cacheKey, result, CACHE_TTL.USERS)
...
export async function updateInvestment(investmentId, updateData) {
  try {
    ...
    // Invalidate caches (invalidate all since we don't know which user owns this investment)
    cache.delete(CACHE_KEYS.ALL_USERS)
