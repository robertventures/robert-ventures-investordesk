...
export async function updateInvestment(investmentId, updateData) {
  try {
    ...
    const appTime = await getCurrentAppTime()
    dbData.updated_at = appTime || new Date().toISOString()
    ...
    if (currentInvestment && updateData.status && currentInvestment.status !== updateData.status) {
      ...
      // Status changed from pending -> active (approved/confirmed)
      if (currentInvestment.status === 'pending' && updateData.status === 'active') {
        // Create investment_approved activity
        await supabase
          .from('activity')
          .insert({
            id: generateTransactionId('INV', investmentId, 'investment_approved'),
            user_id: userId,
        ...
        // Create investment_confirmed activity
        await supabase
          .from('activity')
          .insert({
